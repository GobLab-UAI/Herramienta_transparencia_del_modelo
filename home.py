...
tab1, tab2, tab3 = st.tabs(["Detalles del modelo 📄", "Visión general y métricas de rendimiento 📈", "Datos de entrenamiento 💿"])
tab4, tab5, tab6, tab7 = st.tabs([ "Datos de evaluación 🧪" ,"Análisis cuantitativo #️⃣", "Consideraciones éticas 🧠", "Advertencias y recomendaciones ⚠️"])
...
with tab1:
    ...
    questions["desarrollador_modelo"] = st.text_area("2.¿Qué persona u organización desarrolló el modelo?", value=" Esto puede ser utilizado por todas las partes interesadas para inferir detalles relacionados con el desarrollo del modelo y posibles conflictos de interés.",max_chars=SHORT_STRING)
    questions["version_modelo"] = st.text_input("3.¿Cuál es la versión del modelo?¿Han existido versiones anteriores?", value="Esto es útil para que todas las partes interesadas se informen sobre las técnicas y fuentes de datos que probablemente estuvieron disponibles durante el desarrollo del modelo.",max_chars=SHORT_STRING)
    ...
    questions["tipo_modelo"] = st.text_area("5.¿Qué tipo de modelo es?", value="Esto incluye detalles básicos de la arquitectura del modelo, como si es un clasificador de Naive Bayes, una Red Neuronal Convolucional, etc. Esto es probablemente relevante para desarrolladores de software y modelos, así como para personas conocedoras de aprendizaje automático, para resaltar qué tipos de suposiciones están codificadas en el sistema.",max_chars=SHORT_STRING)
    questions["link_modelo"] = st.text_input("6.¿Dónde se pueden encontrar recursos para obtener más información?", value="Por ejemplo, link a la página institucional",max_chars=LONG_STRING)
    questions["cita_modelo"] = st.text_area("7.¿Cómo debería citar el modelo?", max_chars=LONG_STRING)
...
with tab2:
    ...
    questions["UsoPrevisto_modelo"] = st.text_area("13.¿Cuál es el uso previsto y las expectativas del usuario?", value="Esta sección detalla si el modelo se desarrolló con tareas generales o específicas en mente. Los casos de uso pueden estar definidos de manera amplia o estrecha según lo que los desarrolladores pretendan. Por ejemplo, si el modelo se construyó simplemente para etiquetar imágenes, esta tarea debería indicarse como el caso de uso principal previsto.", max_chars=LONG_STRING)
    questions["UsosNocontext_modelo"] = st.text_area("14.¿Qué usos están fuera del alcance del modelo?", value="Aquí, la ficha del modelo debería resaltar la tecnología con la que el modelo podría confundirse fácilmente, o contextos relacionados a los que los usuarios podrían intentar aplicar el modelo.", max_chars=LONG_STRING)

    st.header("TA: Classificación 🚨")
...
with tab4:
    ...
    questions["preprocesamiento_evaluacion"] = st.text_area("27.¿Cómo se preprocesaron los datos para la evaluación?", value="(por ejemplo, tokenización de oraciones, recorte de imágenes, cualquier filtrado como eliminar imágenes sin caras)", max_chars=LONG_STRING)
...
with tab6:
    ...
    questions["riesgos_uso_modelo"] = st.text_area("36.¿Qué riesgos pueden estar presentes en el uso del modelo?", value="Trate de identificar a los posibles receptores, la probabilidad y la magnitud de los daños. Si no se pueden determinar, indique que se consideraron pero siguen siendo desconocidos.", max_chars=LONG_STRING)
    ...
    questions["otra_consideracion"] = st.text_area("38.De existir alguna otra consideración ética adicional que se haya tenido en cuenta en el desarrollo del modelo, indicar en este apartado.", value="Por ejemplo, revisión por parte de un consejo externo o pruebas con una comunidad específica.", max_chars=LONG_STRING)
...
with tab7:
    ...
    questions["caracteristicas ideales"] = st.text_area("42.¿Cuáles son las características ideales de un conjunto de datos de evaluación para este modelo?", max_chars=LONG_STRING)
